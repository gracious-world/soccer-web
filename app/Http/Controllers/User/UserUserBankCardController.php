...
class UserUserBankCardController extends UserBaseController {
    ...
    protected $resourceView = 'userCenter.userBankCard';
    protected $modelName = 'App\Models\AppUser\UserUserBankCard';
    ...
    protected function beforeRender() {
        ...
        switch ($this->action) {
            case 'index':
                ...
                $aStatus =UserUserBankCard::$validStatuses;
    ...
    private function postDestroy() {
        ...
        if (!$bValidated) {
            return $this->goBack('error', __('_userbankcard.validate-card-fail'));
        } else {
...
//        }
    ...
    public function modifyCard($iStep, $id = null) {
        ...
        switch ($iStep) {
            case 0:
                return $this->validateBankCard($id);
            case 1:
     ...
     * @return [Response]         [description]
     */
    private function validateBankCard($iCardId = null) {
...
//            'account' => 'required|regex:/^[0-9]*$/|between:16,19|confirmed',
        ...
        $oBank = Bank::find($this->params['bank_id']);
        $this->params['bank'] = $oBank->name;
        ...
        $this->action = 'confirm';
        $sValidationString = md5(mt_rand(10000000,99999999));
        $this->setVars('__sValidation', $sValidationString);
        Session::put('__validation_for_bindcard', $sValidationString);
    ...
    private function _bindConfirm() {
        ...
        if (!is_object($oBankCard)) {
            ...
            $oBankCard->status =UserUserBankCard::STATUS_IN_USE;
    ...
    private function _modifyConfirm($iCardId) {
        DB::connection()->beginTransaction();
        //查询银行卡是否存在，如果不存在先保存银行卡信息
        $oBankCard =UserUserBankCard::getObjectByParams(['account' => $this->params['account']]);
        if (!is_object($oBankCard)) {
            $oBankCard = new BankCard;
            $oBankCard->fill($this->params);
            $bSucc = $oBankCard->save();
