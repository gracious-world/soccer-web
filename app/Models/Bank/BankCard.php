...
class BankCard extends BaseModel {
    ...
    protected function beforeValidate() {
        ...
        if ($this->id) {
            static::$rules['account'] = 'required|regex:/^[0-9]*$/|between:16,19' . $this->id;
        }
    ...
    public static function getUserCardsInfo($iUserId, $aColumns = null) {
        // if (!$columns || (is_array($columns) && !count($columns))) $columns = [static::$titleColumn];
        // $aColumns = array_merge(['account'], $columns);
        $aColumns or $aColumns = ['id', 'account'];
        // $iUserId = Session::get('user_id');
        $oQuery = static::getUserCards($iUserId);
