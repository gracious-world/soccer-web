...
//                $fPrize = static::countPrize($oSeries,$this->classic_prize,1960,$aBasicLevel);
//                pr($fPrize);
//            exit;
//            }
...
//        exit;
    ...
    public static function getPrizeGroupByClassicPrize($iClassicPrize, $iSeriesId) {
        ...
        if (static::$cacheLevel != self::CACHE_LEVEL_NONE) {
            ...
            $sCacheKey = static::compilePrizeGroupCacheKey($iClassicPrize, $iSeriesId);
        ...
        if ($bReadDb) {
            $oGroup = static::where('series_id', '=', $iSeriesId)->where('classic_prize', '=', $iClassicPrize)->first();
            if (!is_object($oGroup)) {
                return false;
            }
        }
    ...
    private static function compilePrizeGroupCacheKey($iClassicPrize, $iSeriesId) {
        return static::getCachePrefix(true) . $iSeriesId . '-' . $iClassicPrize;
    }
    ...
    public static function getPrizeGroupByName($sPrizeGroup) {
        if (!$sPrizeGroup)
            return false;
        return static::where('name', '=', $sPrizeGroup)->get(); // ->where('type', '=', $iLotteryType)
    }
     ...
     * @return int | false
     */
    public static function getGroupId($iSeriesId, $sGroupName) {
        $oGroup = static::getObjectByName($iSeriesId, $sGroupName);
        return $oGroup ? $oGroup->id : null;
    }

    public static function compileNameCacheKey($iSeriesId, $sGroupName) {
        return static::getCachePrefix(true) . "$iSeriesId-$sGroupName";
    }

    public static function getObjectByName($iSeriesId, $sGroupName) {
        $iSeriesId = Series::getRealSeriesId($iSeriesId);
        $bReadDb   = true;
        $bPutCache = false;
        if (static::$cacheLevel != self::CACHE_LEVEL_NONE) {
            Cache::setDefaultDriver(static::$cacheDrivers[static::$cacheLevel]);
            $sCacheKey  = static::compileNameCacheKey($iSeriesId, $sGroupName);
        ...
        if ($bReadDb) {
            $oGroup = static::where('series_id', '=', $iSeriesId)->where('name', '=', $sGroupName)->first();
            if (!is_object($oGroup)) {
                return false;
            }
        }
     ...
     * @return array &
     */
    public static function & getPrizeDetailsNew($iGroupId) {
        return PrizeDetail::getDetailsNew($iGroupId);
    }
    ...
    public static function & getPrizeGroupWaterMap() {
        ...
        $aPrizeGroupWaters = static::all($aColumns);
        $data              = [];
    ...
    public static function getPrizeGroupsByParams($aParams, $iSeriesId = null, $aColumns = null) {
        ...
        //     }
        // }
        $oQuery   = static::whereIn('classic_prize', $aParams);
        if ($iSeriesId)
    ...
    public static function getPrizeGroupsWithOnlyKey($aParams, $iSeriesId = null, $aColumns = null) {
        $aGroups      = [];
        $aPrizeGroups = static::getPrizeGroupsByParams($aParams);
    ...
    public static function getPrizeGroupsBySeriesName($iSeriesId, $sName) {
        ...
        $oQuery = static::where('series_id', '=', $iSeriesId)->where('name', '=', $sName);
    ...
    public static function & getPrizeGroups($iSeriesId, $iMaxGroup, $iMinGroup = null, $iCount = null) {
        $aAllPrizeGroups = & static::getAllPrizeGroups($iSeriesId);
        $aGroups         = [];
        ...
        return $aGroups;
//        $oQuery = static::where('series_id', '=', $iSeriesId)->where('classic_prize', '<=', $iMaxGroup);
...
//        return $oQuery->get();
    ...
    protected static function compileAllPrizeGroupCacheKey($iSeriesId) {
        return static::getCachePrefix(true) . 'all-prize-groups';
    }
    ...
    public static function & getAllPrizeGroups($iSeriesId) {
        ...
        if (static::$cacheLevel != self::CACHE_LEVEL_NONE) {
            Cache::setDefaultDriver(static::$cacheDrivers[static::$cacheLevel]);
            $sCacheKey = static::compileAllPrizeGroupCacheKey($iSeriesId);
        ...
        if ($bReadDb) {
            $aGroups      = [];
            $oPrizeGroups = static::where('series_id', '=', $iSeriesId)->orderBy('name', 'asc')->get();
...
//        exit;
    ...
    public static function & getPrizeGroupArray($iSeriesId, $iMaxGroup, $iMinGroup = null, $iCount = null) {
        $aPrizeGroups = [];
        $oPrizeGroups = static::getPrizeGroups($iSeriesId, $iMaxGroup, $iMinGroup, $iCount);
        foreach ($oPrizeGroups as $oPrizeGroup) {
...
//        $oPrizeGroups = static::getPrizeGroups($iSeriesId, $iMaxGroup, $iMinGroup, $iCount);
        $aPrizeGroups = & static::getPrizeGroups($iSeriesId, $iMaxGroup, $iMinGroup, $iCount);
//        $oQuery       = static::where('series_id', '=', $iSeriesId)->where('classic_prize', '<=', $iMaxGroup);
//        empty($iMinGroup) or $oQuery       = $oQuery->where('classic_prize', '>=', $iMinGroup);
//        $oQuery       = $oQuery->orderBy('name', 'asc');
//        empty($iCount) or $oQuery       = $oQuery->limit($iCount);
