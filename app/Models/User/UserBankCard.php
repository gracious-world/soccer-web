...
//        'account_confirmation' => 'required|regex:/^[0-9]*$/|between:16,19',
    ...
    protected function beforeValidate() {
        $this->id or $this->status = self::STATUS_IN_USE;
    ...
    protected static function getUserBankCards($iUserId) {
        $oQuery = static::where('user_id', '=', $iUserId)->where('status', '<>', self::STATUS_DELETED);
        return $oQuery;
    }
    ...
    public static function getUserCardsInfo($iUserId, $aColumns = null) {
        ...
        $oQuery = static::getUserBankCards($iUserId);
...
//        return static::where('id', '=', $this->id)->where('status', '=', self::STATUS_IN_USE)->update($aExtraData) > 0;
    }

    public static function _setStatus($iUserId, $iFromStatus, $aExtraData = []) {
        return static::where('user_id', '=', $iUserId)->where('status', '=', $iFromStatus)->update($aExtraData) > 0;
    }
    ...
    protected function getDisplayAccountNoAttribute() {
        return substr($this->attributes['account'], -6);
    }
