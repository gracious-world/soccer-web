...
class RegisterLink extends BaseModel {
    ...
    protected function getFormattedStatusAttribute() {
        ...
        return __('_registerlink.' . strtolower(Str::slug(static::$aStatuses[$sStatus])));
    }

    protected function getFormattedIsAgentAttribute() {
        $sType = $this->is_agent;
        if ($this->is_top) $sType = User::TYPE_TOP_AGENT;
        return __('_user.' . strtolower(Str::slug(User::$userTypes[$sType])));
    }
    
    protected function getExpireTimeFormattedAttribute(){
        if ($this->valid_days){
            return time() > strtotime($this->attributes['expired_at']) ? '无效' : substr($this->attributes['expired_at'], 0,10);
        }
        else{
            return '永久有效';
        }
    ...
    public static function getTotalUserCountByUserId($iUserId){
        $oQuery = static::where('user_id', '=', $iUserId)->where('is_top','=',0);
    ...
    public function getRegisterPrizeGroups(& $aPrizeSetQuota) {
        $aPrizeSetQuota = json_decode($this->agent_prize_set_quota,true);
        return json_decode($this->prize_group_sets);
    }

    public function createRecord($oUser){
        $data = [
        ...
        $oRegisterLinkUser = new RegisterLinkUser($data);
        if (!$oRegisterLinkUser->save()){
            pr($oRegisterLinkUser->validationErrors->toArray());
            exit;
        }
        return $oRegisterLinkUser;
    }
