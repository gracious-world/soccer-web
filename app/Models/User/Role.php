...
//    public static $aRoleTypes = ['Admin', 'User']; // TIP 'Special' 特殊角色不必要，只需划分管理员和用户, 2014-08-19
    ...
    const EVERY_USER = 1;
    const DENY = 2;
    const TOP_AGENT = 3; // 总代
    const AGENT = 4; // 一代
    const PLAYER = 5; // 玩家
    const DONT_UP_PRIZE = 6; // 升点黑名单
    const DONT_DOWN_PRIZE = 7; // 降点黑名单
    const BONUS_BLACK = 8; // 分红黑名单
    const WITHDRAW_WHITE = 9; // 提现白名单
    const ICBC_DEPOSIT_WHITE = 10; // 工行充值白名单
    const WITHDRAW_BLACK = 11; // 提现黑名单
    const BAD_RECORD = 12; // 用户不良记录
    const ACTIVITY_REGIST_BLACK = 13; // 注册活动黑名单
    //
    //------------roles---------------
    ...
    public static function & getRightsOfRoles($aRoleIds) {
        ...
        foreach ($aRoleIds as $iRoleId) {
            $aRightsOfRole = & static::getRights($iRoleId);
    ...
    public static function & getBlockedFuncsOfRoles($aRoleIds) {
        $aRights = [];
        foreach ($aRoleIds as $iRoleId) {
            if ($aRightsOfRole = & static::getBlockedFuncs($iRoleId)) {
                $aRights = array_merge($aRights, $aRightsOfRole);
            }
        }
        $aRights = array_unique($aRights);
...
//        exit;
        if (static::$cacheLevel == self::CACHE_LEVEL_NONE) {
            $obj = static::find($iRoleId);
            $aRights = & $obj->explodeBlockedFuncs();
        } else {
            $key = static::getBlockedCacheKey($iRoleId);
//            pr($key);
//            exit;
...
//                pr($obj->toArray());
            ...
            $key = static::getRightsCacheKey($iRoleId);
            Cache::setDefaultDriver(static::$cacheDrivers[static::$cacheLevel]);
...
//        exit;
    ...
    public static function getRightsCacheKey($iRoleId) {
        return static::getCachePrefix() . 'rights-' . $iRoleId;
    }
    ...
    public static function getBlockedCacheKey($iRoleId) {
        return static::getCachePrefix() . 'blocked-' . $iRoleId;
    }
    ...
    protected function afterSave($oSavedModel) {
        ...
        static::deleteRightCache($this->id);
//            return parent::afterSave($oSavedModel);
    ...
    protected static function deleteRightCache($mRoleId = null) {
        ...
        foreach ($aRoleId as $iRoleId) {
            $sCacheKey = static::getRightsCacheKey($iRoleId);
