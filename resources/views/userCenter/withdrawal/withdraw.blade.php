...
<div class="recharge-box">
    ...
    @endif
    <form action="{!! route('user-withdrawal.withdraw', 1) !!}" method="post" id="J-form" class="clearfix">
        <input type="hidden" name="_token" value="{!! csrf_token() !!}" />
        <input type="hidden" name="step" value="1" />
        <table width="70%" class="table-field left">
            <tr>
                    ...
                    @foreach($aBankCards as $oBankCard)
                        <div class="card-info-list">
                            <label class="img-bank">
                                ...
                                <span>尾号：{!! substr($oBankCard->account,12)!!}</span>
    ...
    <div class="recharge-help">
            ...
            <div class="prompt c-important">
                 提取余额到银行卡<br/>
                 每天02:00AM-08:00AM累计提现不能超过5万；单日提现次数最多为5次。
            </div>
            <p class="row-desc alert-message withdraw-time"><i class="alert-icon"></i><span>今日剩余提现次数：<span class="c-black">{!! $iWithdrawLimitNum - $iWithdrawalNum !!}/{!! $iWithdrawLimitNum !!}</span></span></p>
        </div>
</div>
...
(function($){
    ...
    var ipt1 = $('#J-input-money'),
        ...
        tip = new gagame.Tip({cls:'j-ui-tip-b j-ui-tip-input-floattip'}),
...
var addCardMask = new gagame.Mask(),
    addCardMiniwindow = new gagame.MiniWindow({ cls: 'w-12 add-card-miniwindow' });
