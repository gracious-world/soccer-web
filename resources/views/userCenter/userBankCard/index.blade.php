...
<div class="content">
    ...
    <table class="table">
        ...
        <tr>
            ...
            @if($bLocked==0)
            <td><a href="{!!route('bank-cards.modify-card', [0,$data->id])!!}" >修改</a>  |  <a href="{!! route('bank-cards.destroy', $data->id) !!}">删除</a></td>
            @endif
        </tr>
        @endforeach
    </table>
    ...
    <div class="area-search" style="margin-top:10px;">
        <p class="row">
            @if(isset($iBindedCardsNum) && (int)$iBindedCardsNum < $iLimitCardsNum)
                <a  class="btn" href="{!! route('bank-cards.bind-card', 0) !!}" data-add-bankcard>+ 增加绑定</a>
            @endif
            <!-->
            <!-->
            <!-->
