...
@section ('main')
    ...
    <div class="content recharge-netbank">
        <div class="recharge-box">
            <form action="{!! route('user-recharges.quick', $oPlatform->id) !!}" method="post" id="J-form">
                <input type="hidden" name="deposit_mode" value="{!!$iDepositMode!!}" >
                                    ...
                                    <div class="bank-list" id="J-bank-list">
                                        ...
                                        @foreach($oAllBanks as $oBank)
                                            <label class="img-bank" for="J-bank-name-{!! $oBank->bank_identifier !!}">
                                                <span data-id="{!! $oBank->id !!}" class="ico-bank {!! $oBank->bank_identifier !!}">{!! $oBank->bank_name !!}</span>
                                            </label>
                                        @endforeach
    ...
    <script>
        (function($){
                    <!-->
                    @if(!$bSetFundPassword)
            var msg = gagame.Message.getInstance();
            msg.show({
        ...
        location.href = "/";
            ...
            buildingView = function(bankData){
                ...
                $('#J-money-max').text(gagame.util.formatMoney(Number(bankData['max'])));//{!!$fMaxLoad!!}
                $('#J-input-money').val('');
...
//        v = v > maxNum ? maxNum : v;
                if (v){
                    this.value = (this.value == '' ) ? '' : gagame.util.formatMoney(v).replace(/,/g, '');
