 ...
 -->
            <div class="hongbao-box">
                <div class="hb-top">
                    ...
                    <div class="filter-tabs">
                        <div class="filter-tabs-cont">
                            <a @if(Route::current()->getName() == 'user-activity-user-bonuses.index')class="current" @endif href="{!!route('user-activity-user-bonuses.index')!!}?type=pic">全部({!!$iHongBaoTotalCount!!})</a>
                            <a @if(Route::current()->getName() == 'user-activity-user-bonuses.available')class="current" @endif href="{!!route('user-activity-user-bonuses.available')!!}?type=pic">可领取({!!$iHongBaoAvailableCount!!})</a>
                            <a @if(Route::current()->getName() == 'user-activity-user-bonuses.unavailable')class="current" @endif href="{!!route('user-activity-user-bonuses.unavailable')!!}?type=pic">暂不可领取({!!$iHongBaoUnAvailableCount!!})</a>
                            <a @if(Route::current()->getName() == 'user-activity-user-bonuses.received')class="current" @endifhref="{!!route('user-activity-user-bonuses.received')!!}?type=pic">已领取({!!$iHongBaoReceivedCount!!})</a>
                            <a @if(Route::current()->getName() == 'user-activity-user-bonuses.expired')class="current" @endifhref="{!!route('user-activity-user-bonuses.expired')!!}?type=pic">已过期({!!$iHongBaoExpiredCount!!})</a>
                        </div>
                    </div>
                </div>
                @if($datas->getTotal()>0)
                ...
                <ul class="hb-list clearfix" id="J-hb-list" data-show-mode="view" style="display:none;">
                    ...
                    // 过期的
                    else if( $data->status == ActivityUserBonus::STATUS_EXPIRED ){
                        $statusClass .= ' hb-locked';
                        $btnHtml1 = '<a class="btn" href="javascript:void(0);">已过期</a>';
                        $btnHtml2 = '进行中';
                    }
                    ?>
                        ...
                        <div class="css-flip css-flip-x">
                            ...
                            <div class="flip-back">
                                <dl>
                                    ...
                                    <dt>获得自：</dt>
                                    <dd><span class="c-important">{!!$data->activity_name!!}</span></dd>
                                    <dt>领取过期时间：</dt>
                                    <dd><span class="c-important">{!!$data->expired_at!!}</span></dd>
                                </dl>
                            </div>
                        </div>
...
(function($){
    ...
    // 下拉组件
    // var selectYear = new gagame.Select({
