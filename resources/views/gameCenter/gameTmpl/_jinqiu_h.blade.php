<script id="gamesHistoryList" type="text/x-dot-template">
 @{{ var ttgWays = Mango.Config.gameConfig.way_odds.ttg[0], gameIndexNum = 0; }}
    @{{ for (var n in it) { var groupItem = it[n], bet_date = n.replace(/-/g, ''); }}
        ...
        <dt>@{{=n}} @{{=groupItem[0].week}} 12:00:00 - 次日 12:00:00 &nbsp;&nbsp;<span class="matchSize">0</span> 场比赛可投注
            ...
            </dt>@{{ for(var i=0,l=groupItem.length; i<l;i++) { var item = groupItem[i]; var gameData = JSON.stringify({bn: item.bn, single: item.single, week: item.week, num: item.num, h_cn: item.h_cn_abbr, a_cn: item.a_cn_abbr}); var tip = '截止时间:' + item.sale_stop_at + '; 开赛时间:' + item.date + ' ' + item.time; var tipData = [item.time, item.sale_stop_at.split(' ')[1]].join(';'); }}
        <dd game="@{{=item.bn}}" data=@{{=gameData}} game_index="@{{=gameIndexNum}}" bet_date="@{{=bet_date}}" class="league_470 isStop">
            ...
            <span class="co6 btnBox  over">
                <div class="line1">
                    @{{ for(var hn=0, hl = ttgWays.length; hn<hl; hn++) { var name = ttgWays[hn], hcn = Mango.Config.gameConfig.way_odd_map[name]['name']; }}
                    <em class="@{{=(name == item.ttg ? 'c_f6c15a' : (hn == hl-1 ? 'lastOne' : ''))}}">
                        @{{=(name == item.ttg ? hcn : '--')}}
                    </em>
                    @{{ } }}
                </div>
